...
from tensorflow.keras.layers import Flatten, Lambda, ELU, Cropping2D
...
def generator(samples, batch_size=32, prob_flip=1.0, seed=None):
    ...
    Args:
        ...
        seed (int): Random seed for reproducibility.  Specify None for
                    default behaviour.
    """
    def send_off_batch(X, y, r):
        """Internal method to send off a batch when required"""
    ...
    while True: # Loop forever so the generator never terminates
        ...
        # For each file...
        for filename, steering in samples_copy:
            ...
            # Do we send off a batch?
            if len(X) >= batch_size:
                yield from send_off_batch(X, y, r)
                X = []
                y = []            
            ...
            if prob_flip is not None:
                un = r.random() if seed is not None else random()
                if un <= prob_flip:
        ...
        # Send off the last batch if it doesn't completely match
        # the batch size
        if len(X) != 0:
            yield from send_off_batch(X, y, r)
...
def define_model():
    ...
    model.add(Cropping2D(cropping=((70, 20), (0, 0)), name='cropping'))
...
def main(data_dir, model_checkpoint_dir='./models', model_output_dir='./checkpoint',
         ...
         prob_flip=1.0, batch_size=32, num_epochs=20, plot_loss=True, seed=42):
    ...
    # shuffle anything.
    # However, we can set it to 1 because we want to flip the images as well
    # as present the original - this is reproducible
    validation_generator = generator(validation_samples, batch_size=batch_size,
                                     prob_flip=1, seed=None)
    ...
    num_cycles = 2 if prob_flip == 1 else 1
    history = model.fit(x=train_generator, y=None,
                steps_per_epoch=num_cycles * ceil(len(train_samples) / batch_size),
                validation_data=validation_generator,
                validation_steps=num_cycles * ceil(len(validation_samples) / batch_size),
    ...
    test_loss = model.evaluate(x=test_generator, y=None, batch_size=batch_size,
                               verbose=1, steps=num_cycles * ceil(len(test_samples) / batch_size))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prob-flip", type=float, default=1.0, help="Probability for horizontally flipping an image (augmentation)")
